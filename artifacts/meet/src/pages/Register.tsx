...
export default function Register() {
        ...
        try {
          ...
          if (btnEl) {
            if (googleClientId === "335439563229-placeholder.apps.googleusercontent.com") {
              btnEl.innerHTML = `
                <button id="mock-google-signup-btn" type="button" class="w-full h-11 flex items-center justify-center gap-3 bg-white hover:bg-zinc-100 text-zinc-900 border border-zinc-300 font-semibold px-4 rounded-xl transition-all duration-200">
                  ...
                  <span>Sign up with Google (Demo Mode)</span>
                </button>
              `;

              const mockBtn = document.getElementById("mock-google-signup-btn");
              if (mockBtn) {
                mockBtn.onclick = () => {
                  const email = prompt("Enter a mock Google email to register/login:", "google-user@example.com");
                  if (email) {
                    setIsGoogleLoading(true);
                    googleLoginMutation.mutate(
                      { data: { idToken: `mock_google_token_${email}` } },
                      {
                        onSuccess: (res) => {
                          ...
                          toast({
                            ...
                            description: `Registered/Logged in via mock Google as ${res.user.name}`,
                          });
                          setLocation("/");
                        },
                        onError: (err: any) => {
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center bg-[#09090b] relative overflow-hidden py-10 px-4">
      ...
      <Card className="w-full max-w-md bg-card/65 backdrop-blur-xl border border-white/10 shadow-2xl relative z-10 overflow-hidden">
        ...
        <CardContent>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <Button
              type="submit"
              className="w-full h-11 rounded-xl text-sm font-medium bg-primary text-primary-foreground hover:bg-primary/95 mt-6 gap-2 shadow-lg shadow-primary/20 transition-all duration-200"
