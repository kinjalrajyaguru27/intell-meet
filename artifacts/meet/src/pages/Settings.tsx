...
import { Settings as SettingsIcon, Globe, Palette, BellRing, Video, Lock, Cog } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Label } from "@/components/ui/label";
import { Switch } from "@/components/ui/switch";
import { useToast } from "@/hooks/use-toast";

type SettingsTab = "general" | "theme" | "language" | "notifications" | "meeting" | "security";
...
export default function Settings() {
  ...
  const handleSave = () => {
    toast({
      ...
      description: "Your configurations have been successfully updated in the database.",
    });
  };
    ...
    <div className="flex-1 flex flex-col min-h-0 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/5 pb-4">
        <div className="flex items-center gap-3">
          ...
          <h1 className="font-semibold text-lg text-white font-sans">Settings Panel</h1>
        </div>
      </div>

      <div className="flex flex-col lg:flex-row gap-6 items-start">
        {/* Left Side Tab Navigation */}
        <div className="w-full lg:w-60 bg-card border border-white/5 p-2 rounded-2xl space-y-0.5 shrink-0">
          {([
            ...
            { value: "meeting", label: "Meeting Settings", icon: Video },
        ...
        <div className="flex-1 w-full bg-card border border-white/5 rounded-2xl p-6">
          <div className="space-y-6 min-h-[300px]">
            
            {activeTab === "general" && (
              <div className="space-y-4">
                <div className="border-b border-white/5 pb-2">
                  <h3 className="text-sm font-bold text-white">General Configurations</h3>
                  <p className="text-xs text-muted-foreground">Adjust productivity tracking and workspace options.</p>
                </div>
                <div className="space-y-3 pt-2">
                  <div className="flex items-center justify-between">
                    <Label htmlFor="analytics-opt" className="text-xs text-zinc-300">Share anonymous diagnostic insights</Label>
                    <Switch id="analytics-opt" checked={analyticsEnabled} onCheckedChange={setAnalyticsEnabled} />
                  </div>
                  <div className="flex items-center justify-between">
                    <Label htmlFor="offline-opt" className="text-xs text-zinc-300">Enable offline caching features</Label>
            ...
            {activeTab === "theme" && (
              <div className="space-y-4">
                <div className="border-b border-white/5 pb-2">
                  <h3 className="text-sm font-bold text-white">Visual Appearance</h3>
            ...
            {activeTab === "language" && (
              <div className="space-y-4">
                <div className="border-b border-white/5 pb-2">
                  ...
                  <p className="text-xs text-muted-foreground">Choose your locale for layouts and notifications formatting.</p>
                </div>
                <div className="space-y-2 pt-2">
                  {(["English", "Español", "Français", "Deutsch", "Hindi"] as const).map(lang => (
                    <button
                      key={lang}
                      onClick={() => setSelectedLang(lang)}
                      className={`w-full text-left px-4 py-2.5 rounded-lg border text-xs font-semibold transition-all ${
                        selectedLang === lang
                          ? "border-primary bg-primary/5 text-primary"
                          : "border-white/5 bg-white/5 hover:bg-white/10 text-zinc-400"
                      }`}
            ...
            {activeTab === "notifications" && (
              <div className="space-y-4">
                ...
                <div className="space-y-3 pt-2">
                  <div className="flex items-center justify-between">
                    <Label htmlFor="email-opt" className="text-xs text-zinc-300">Deliver email digests weekly</Label>
                    <Switch id="email-opt" checked={emailAlerts} onCheckedChange={setEmailAlerts} />
                  </div>
                  <div className="flex items-center justify-between">
                    <Label htmlFor="push-opt" className="text-xs text-zinc-300">Display browser push notifications</Label>
                    <Switch id="push-opt" checked={pushAlerts} onCheckedChange={setPushAlerts} />
                  </div>
                  <div className="flex items-center justify-between">
                    <Label htmlFor="mention-opt" className="text-xs text-zinc-300">Alert on workspace mentions</Label>
            ...
            {activeTab === "meeting" && (
              <div className="space-y-4">
                <div className="border-b border-white/5 pb-2">
                  ...
                  <p className="text-xs text-muted-foreground">Set default audio and video rules for joining room lobbies.</p>
                </div>
                <div className="space-y-3 pt-2">
                  <div className="flex items-center justify-between">
                    <Label htmlFor="mute-opt" className="text-xs text-zinc-300">Auto-mute microphone when entering room</Label>
                    <Switch id="mute-opt" checked={autoMute} onCheckedChange={setAutoMute} />
                  </div>
                  <div className="flex items-center justify-between">
                    <Label htmlFor="camera-opt" className="text-xs text-zinc-300">Disable camera when joining conferences</Label>
                    <Switch id="camera-opt" checked={autoCameraOff} onCheckedChange={setAutoCameraOff} />
                  </div>
                  <div className="flex items-center justify-between">
                    <Label htmlFor="quality-opt" className="text-xs text-zinc-300">Enable high-definition video capture</Label>
                    <Switch id="quality-opt" checked={highQualityVideo} onCheckedChange={setHighQualityVideo} />
                  </div>
                </div>
              </div>
            )}
          ...
          <div className="flex justify-end border-t border-white/5 mt-6 pt-4">
