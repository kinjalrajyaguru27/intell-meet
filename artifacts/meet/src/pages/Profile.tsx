          ...
          try {
            ...
            if (btnEl) {
              if (googleClientId === "335439563229-placeholder.apps.googleusercontent.com") {
                btnEl.innerHTML = `
                  <button id="mock-google-link-btn" type="button" class="h-9 flex items-center justify-center gap-2 bg-white hover:bg-zinc-100 text-zinc-900 border border-zinc-300 font-semibold px-4 rounded-xl transition-all duration-200 text-xs">
                    ...
                    <span>Link Google (Demo Mode)</span>
                  </button>
                `;

                const mockBtn = document.getElementById("mock-google-link-btn");
                if (mockBtn) {
                  mockBtn.onclick = () => {
                    const email = prompt("Enter a mock Google email to link:", profile?.email || "google-user@example.com");
                    if (email) {
                      googleLoginMutation.mutate(
            ...
            <CardContent className="pt-8">
              <div className="flex flex-col md:flex-row items-center md:items-start gap-6">
                ...
                <div className="relative shrink-0">
                  <Avatar className="w-24 h-24 rounded-full border-4 border-primary/20 shadow-xl">
                    {profile.avatar ? (
                      <img src={profile.avatar} alt={profile.name} className="w-full h-full object-cover rounded-full" />
                    ) : (
                      <AvatarFallback className="bg-gradient-to-tr from-violet-600 to-indigo-600 text-white font-bold text-3xl">
                ...
                <div className="flex-1 text-center md:text-left space-y-3">
                  ...
                  <p className="text-sm text-zinc-400 font-medium">
                    ...
                    {profile.department && <span className="text-zinc-600"> • </span>} 
                    <span className="text-primary">{profile.department || ""}</span>
                  </p>

                  <p className="text-xs text-zinc-400 max-w-xl leading-relaxed italic">
            ...
            <Card className="bg-card/30 border-white/5 backdrop-blur-md">
              ...
              <CardContent className="pt-4 space-y-4 text-xs">
                ...
                <div className="flex items-center justify-between text-zinc-400">
                  ...
                  <span className="text-white font-semibold">{profile.phoneNumber || "Not Specified"}</span>
                </div>
                <div className="flex items-center justify-between text-zinc-400">
                  <span className="flex items-center gap-2 font-medium">
                  ...
                  <span className="text-white font-semibold">{profile.timezone || "UTC"}</span>
                </div>
                <div className="flex items-center justify-between text-zinc-400">
                  <span className="flex items-center gap-2 font-medium">
                  ...
                  <span className="text-white font-semibold">{formatDate(profile.createdAt)}</span>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="bg-card/30 border-white/5 backdrop-blur-md">
              ...
              <CardContent className="pt-4 space-y-4 text-xs">
                <div className="flex items-center justify-between">
                  <span className="text-zinc-400 font-medium">Email Alerts</span>
                ...
                <div className="flex items-center justify-between">
                  <span className="text-zinc-400 font-medium">Push Notifications</span>
                ...
                <div className="flex items-center justify-between">
                  <span className="text-zinc-400 font-medium">SMS Notifications</span>
          ...
          <CardContent className="pt-4 space-y-4">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 rounded-xl bg-black/20 border border-white/5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="text-sm font-semibold text-white">Standard Login</h4>
                  <p className="text-xs text-zinc-400">
                    {profile.hasPassword 
              ...
              <div>
                <Button
                  ...
                  size="sm"
                  onClick={() => setLocation("/profile/edit")}
                  className="text-xs border-white/10 hover:bg-white/5 text-white h-9 px-4 rounded-xl"
                >
